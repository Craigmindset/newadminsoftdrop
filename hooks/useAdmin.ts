...
export default function useAdmin(
  ...
  async function getSenders(page: number, limit: number) {
    ...
    let res = await getData(
      `${ENDPOINTS.users}?role=SENDER&page=${page}&limit=${limit}`,
      refreshAuth,
      authToken,
    );
  ...
  async function getCarriers(page: number, limit: number) {
    setAdminLoading(true);
    let res = await getData(
      `${ENDPOINTS.users}?role=CARRIER&page=${page}&limit=${limit}`,
      refreshAuth,
      authToken,
    );
  ...
  async function getSingleUser(userId: string) {
    ...
    console.log(
      "result from gettign single user",
      res.error?.response?.data || res.data?.data,
    );
    if (res.err) {
      handleErrorCase(res, setAdminLoading, logout, setError);
      return;
  ...
  async function toggleGuarantorOrVehicleDetails(
    id: string,
    vehicleType: string,
    body: any,
  ) {
    ...
    if (vehicleType === "walking" || vehicleType === "bicycle") {
      res = await patchData(
        `/admin/carrier/${id}/guarantor-details`,
        refreshAuth,
        body,
        authToken,
      );
    ...
    if (vehicleType === "bike" || vehicleType === "car") {
      res = await patchData(
        `/admin/carrier/${id}/vehicle-details`,
        refreshAuth,
        body,
        authToken,
      );
