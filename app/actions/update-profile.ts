...
export async function updateSenderProfile(formData: FormData) {
  try {
    ...
    if (profileImage) {
      try {
        // First, check if any buckets exist
        ...
        if (bucketsError) {
          ...
          return {
            success: false,
            error: "Unable to access storage. Storage might not be enabled for this project.",
          }
        }

        // If no buckets exist, we can't proceed with image upload
